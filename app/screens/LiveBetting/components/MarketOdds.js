...
let previousMatchMarketData;
let previousMatchMarketOddData;
// var previousMatchData;

const colorChange = (colorFlag) => {
  if (colorFlag === 1) {
    return { borderBottomWidth: spacing.extraSmall, borderBottomColor: UIColors.appRedColor };
  } else if (colorFlag === 2) {
    return { borderBottomWidth: spacing.extraSmall, borderBottomColor: UIColors.appGreenColor };
  }
  return { borderBottomWidth: spacing.extraSmall, borderBottomColor: UIColors.greyBackground };
};
...
const MarketOdds = props => (
  <View style={styles.listContainer}>
    <FlatList
      ...
      renderItem={(item) => {
        const x = _.find(props.previousMatchData, { id: item.item.id });
        if (x && x.specifiers && x.specifiers[0]) {
          previousMatchMarketData = x.specifiers[0].outcomes;
        }
        return (
          <View>
                      ...
                      <View style={styles.specifierContainer}>
                        ...
                        <FlatList
                          ...
                          renderItem={(dataValue) => {
                            ...
                            previousMatchMarketOddData = _.find(previousMatchMarketData, { id: dataValue.item.id });
                            let colorFlag = 0;
                            if (previousMatchMarketOddData && dataValue.item.odds !== previousMatchMarketOddData.odds) {
                              dataValue.item.odds < previousMatchMarketOddData.odds ? (colorFlag = 1) : (colorFlag = 2);
                            } else {
                              colorFlag = 0;
                            }
                            ...
                            const isContains = !_
                              .chain(props.betSlips)
                              .find({ id: dataValue.item.id, marketID: dataValue.item.marketID, matchID: props.matchID })
                              .isEmpty()
                              .value();
                            return (
                              <View style={styles.oddsContent}>
                                ...
                                {(dataValue.item.status === 'open'|| dataValue.item.status === 'suspended, open' || dataValue.item.status === 'open, suspended' || dataValue.item.status === '1') ?
                                  <View style={{ flex: 1, width: '100%', height: itemSizes.defaultButtonHeight }}>
                                    {(isMarketRepeated && !isContains) ?
                                      <TouchableOpacity
                                        disabled
                                        // eslint-disable-next-line no-nested-ternary
                                        style={[styles.oddButtonStyle, { backgroundColor: UIColors.lightGreyBackground, opacity: 0.4 }, (colorFlag && props.colorTimer) ? colorFlag === 1 ?
                                          { borderBottomWidth: spacing.extraSmall, borderBottomColor: UIColors.appRedColor } :
                                          { borderBottomWidth: spacing.extraSmall, borderBottomColor: UIColors.appGreenColor } : {}]}
                                      ...
                                      <TouchableOpacity
                                        // eslint-disable-next-line no-nested-ternary
                                        style={[styles.oddButtonStyle, (colorFlag && props.colorTimer) ? colorFlag === 1 ?
                                          { borderBottomWidth: spacing.extraSmall, borderBottomColor: UIColors.appRedColor } :
                                          { borderBottomWidth: spacing.extraSmall, borderBottomColor: UIColors.appGreenColor } : {}]}
                                        onPress={() => props.onPressOdd(item.item.marketUID, dataValue.item)}
                                      >
                                  ...
                                  >
                                    <View
                                      style={[styles.oddsBox, { justifyContent: 'center', alignItems: 'center' }]}
                                    >
                                      <Image
...
// ((dataValue.item.odds>previousMatchMarketOddData) ? [styles.oddsBoxSelected, {borderBottomColor: UIColors.appGreenColor, borderBottomWidth: spacing.border}]: [styles.oddsBoxSelected, {borderBottomColor: UIColors.appRedColor, borderBottomWidth: spacing.border}])
MarketOdds.propTypes = {
  onPressOdd: PropTypes.func,
  marketDataForMatch: PropTypes.array,
  previousMatchData: PropTypes.array,
